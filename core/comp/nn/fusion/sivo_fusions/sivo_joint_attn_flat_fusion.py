...
class SiVoOpMaskJointConcatAttentionFusion(SeqinVecoutFusion):
    ...
    def __init__(
        ...
        assert merge_method in ['add', 'mul', 'cat', 'sub','rsub']
    ...
    def _reduce_wrapping_dim(self, features, org_shape, pad_mask):
        ...
        for dim in range(wrapping_dim):
            ...
            # 修改
            # features = self.hunk_reduce(features, mask=hunk_mask)
            # 对[2,3,768]中一个提交下的所有文件的特征进行相加/
            features = features.masked_fill(~hunk_mask.unsqueeze(-1), 0.0)  # [B, L, D]
            features = features.sum(dim=1)  # [B, D]
        return features
    ...
    def forward(self,
        ...
        self_attn_out, self_attn_mask,del_unmasked_sorted_mask_idxes,add_unmasked_sorted_mask_idxes = self.op_mask_self_attention(add_code_features, del_code_features,
        ...
        if self.reduce_before_attention:
            ...
            add_fused_out = self.query_attention(self_attn_query, add_code_features, add_code_features, add_pad_mask,add_unmasked_sorted_mask_idxes,1)
            del_fused_out = self.query_attention(self_attn_query, del_code_features, del_code_features, del_pad_mask,del_unmasked_sorted_mask_idxes,0)


        else:
            # NOTE: hunk reduce before attention will not work when "reduce_before_attention=False".
            # 1表示是新增  0表示删除
            add_fused_out = self.query_attention(self_attn_out, add_code_features, add_code_features, add_pad_mask,add_unmasked_sorted_mask_idxes,1)
            del_fused_out = self.query_attention(self_attn_out, del_code_features, del_code_features, del_pad_mask,del_unmasked_sorted_mask_idxes,0)
            # add_fused_out = self.query_attention(self_attn_out, add_code_features, add_code_features)
            # del_fused_out = self.query_attention(self_attn_out, del_code_features, del_code_features)

            # 修改
            # pad_len = add_code_features.size(1) - self_attn_out.size(1)
            # self_attn_out1 = torch.nn.functional.pad(self_attn_out,(0,0,0,pad_len))
            # self_attn_out1 = self_attn_out1.unsqueeze(1)
            # add_code_features1 = add_code_features.unsqueeze(1)
            # model = MSCXAttention.MSC_Modified(dim=1, num_heads=1).cuda()
            # output = model(self_attn_out1, add_code_features1)
