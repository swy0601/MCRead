...
class OpMaskJointSeq2Seq(torch.nn.Module):
    ...
    def collect_unmasked_elems(self,
        ...
        unmasked_sorted_mask_idxes = sorted_mask_idxes.clone()
        ...
        unmasked_sorted_mask_idxes = unmasked_sorted_mask_idxes[:, :batch_max_unmasked_len]
        return sorted_feature, sorted_mask,unmasked_sorted_mask_idxes
    def forward(self,
...
class OpMaskJointConcatSelfAttention(OpMaskJointSeq2Seq):
    ...
    def _transformer_forward(self,
        ...
        if S == 0:
            features = torch.zeros(B, 1, D, device=device)
            pad_mask = torch.zeros(B, 1, dtype=torch.bool, device=device)
            attn_mask = None

        attn_mask = self._convert_pytorch_mask(attn_mask)
        pad_mask = self._convert_pytorch_mask(pad_mask)

        features = self._maybe_add_pos_encoding(features)
        features = features.transpose(0, 1).contiguous()  # [S, B, D]
    ...
    def forward(self,
        ...
        add_sorted_features, add_sorted_mask,add_unmasked_sorted_mask_idxes = self.collect_unmasked_elems(add_code_features, add_op_mask, 0)
        del_sorted_features, del_sorted_mask,del_unmasked_sorted_mask_idxes = self.collect_unmasked_elems(del_code_features, del_op_mask, 1)
        ...
        joint_out = joint_features
        return joint_out, joint_mask,del_unmasked_sorted_mask_idxes,add_unmasked_sorted_mask_idxes
