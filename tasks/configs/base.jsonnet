...
local token_pretrained_model = "/home/user/wyh/project/codebert_base";
local character_pretrained_model = "/home/user/wyh/project/canine-s";
local pretrained_model = "/home/user/wyh/project/canine-s";

local additional_special_tokens = [];

local seed = 6324;
local char_max_tokens = 512;  
local max_tokens = 512;
{
  ...
  dataset_reader: {
    type: "imp_flat_combined",
    code_tokenizer: {
      type: "pretrained_transformer",
      model_name: pretrained_model,
      max_length: char_max_tokens,
      tokenizer_kwargs: {
    ...
    char_tokenizer: {
      type: "pretrained_transformer",
      model_name:character_pretrained_model,
      max_length: char_max_tokens,
      tokenizer_kwargs: {
        additional_special_tokens: additional_special_tokens
      }
    ...
    token_tokenizer: {
      type: "pretrained_transformer",
      model_name: token_pretrained_model,
  ...
  model: {
    type: "hybrid_imp_seqin_classifier",
    code_embedder: {
      token_embedders: {
        code_tokens: {
          type: "pretrained_transformer",
    ...
    fusion: {
      ...
      //原先keep_unmasked_order: false,
      keep_unmasked_order: true,
  ...
  trainer: {
    ...
    num_epochs: 10,
    patience: null,
    cuda_device: 0,                 # todo: change this if needed
