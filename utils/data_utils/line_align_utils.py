        ...
        for hunk in one_diff:
            ...
            for i, line in enumerate(lines):
                ...
                if line_idx == max_lines or len(aligned_tokenized_lines) >= diff_max_tokens:
                    return return_line_aligned_single_stream(aligned_tokenized_lines, aligned_line_idxes,
                ...
                if line[-1] == 'insert':
                    if data_type == 'add':
                        tokenized_line = pretrained_transformer_tokenize_and_filter_line(line[1], tokenizer,
                                                                                         keep_tokenizer_head_tail_token)
                        ...
                        if len(aligned_tokenized_lines) + len(tokenized_line) >= diff_max_tokens:
                            ...
                            aligned_line_idxes.extend(
                                generate_line_gather_idx(tokenized_line, line_idx, line_max_tokens))
                    else:
                ...
                # No add, only remove
                    ...
                    if data_type == 'del':
                        tokenized_line = pretrained_transformer_tokenize_and_filter_line(line[0], tokenizer,
                                                                                         keep_tokenizer_head_tail_token)
                        tokenized_line = maybe_add_line_separator(tokenized_line, line_separator)
                        # pre-check length constraint before extending
                        if len(aligned_tokenized_lines) + len(tokenized_line) >= diff_max_tokens:
                ...
                # Lines replace or match line whatever 'equal' or 'replace'
                    ...
                    tokenized_line = pretrained_transformer_tokenize_and_filter_line(line[fetch_idx], tokenizer,
                                                                                     keep_tokenizer_head_tail_token)
                    tokenized_line = maybe_add_line_separator(tokenized_line, line_separator)
                    # pre-check length constraint before extending
                    if len(aligned_tokenized_lines) + len(tokenized_line) >= diff_max_tokens:
                ...
                if insert_empty_line_placeholder:
                    ...
                    line_placeholders = get_empty_line_placeholders(tokenizer, keep_tokenizer_head_tail_token,
                                                                    line_separator,
                                                                    empty_line_placeholder)  # [Token(empty_line_placeholder), Token(line_separator)]
                    if line_empty_flag:
                        ...
                        aligned_line_idxes.extend(
                            generate_line_gather_idx(line_placeholders, line_idx, line_max_tokens))
...
def align_lines_in_diff_jointly1(diff):
    ...
    for hunks in diff:
        for hunk in hunks:
            ...
            # aligned_lines = [ [old_line, new_line, tag], ... ]
            aligned_lines = construct_diff_sequence(hunk['removed_code'], hunk['added_code'])

            new_removed = []
            new_added = []

            # 遍历 aligned_lines，将 insert / delete 写回 hunk
            for old_line, new_line, tag in aligned_lines:

                if tag == "equal":
                    # 两边保持一致
                    new_removed.append(old_line)
                    new_added.append(new_line)

                elif tag == "replace":
                    # replace 的 old_line / new_line 已经由 construct_diff_sequence 对齐
                    new_removed.append(old_line)
                    new_added.append(new_line)
            ...
            # 将补齐后的 removed_code 和 added_code 回写 hunk
            hunk['removed_code'] = new_removed
            hunk['added_code'] = new_added
